...
public class WorkOrderQueryUtil {
	...
	public static final String getWorkOrderList = " select wh.workorder_no as workorderNo,customer_code as customer,sales_order_no as  salesOrderNo ,wd.remarks as remarks,om.uom as uom from workorder_hdr wh left join workorder_dtl wd on wd.workorder_no = wh.workorder_no left join uom om on om.uom_id = wd.uom_id order by wh.workorder_no desc ";
	
	public static final String INSERT_WORKORDER_HDR = "INSERT INTO workorder_hdr (workorder_no,customer_code,workorder_date,sales_order_no,created_by,created_on) "
			+ " values (:workorderNo,:customer,to_date(:workorderDate,'DD/MM/YYYY'),:salesOrderNo,'E0001',now()) returning workorder_no as workorderNo";
	...
	public static final String INSERT_WORKORDER_DTL = "INSERT INTO workorder_dtl (workorder_no,item_id,quantity,uom_id,delivery_date,remarks,created_by,created_on) "
			+ " values (:workorderNo,:itemId,:quantity,:uomId,to_date(:deliveryDateObj,'DD/MM/YYYY'),:remarks,'E0001',now()) ";
	...
	public static final String SELECT_SALES_QUOTE = "select customer_code as customer,sales_order_no as salesOrderNo from sales_order_hdr where customer_code=?";
	
	public static String SELECT_WORKORDER_HDR = "select workorder_no as workorderNo,customer_code as customer,workorder_date as workorderDate,sales_order_no as salesOrderNo from workorder_hdr where workorder_no=?";
	
	public static String UPDATE_WORKORDER_HDR = "update workorder_hdr set customer_code = ?, workorder_date=to_date(?,'DD/MM/YYYY'),sales_order_no=? where workorder_no=? Returning workorder_no as workorderNo";
